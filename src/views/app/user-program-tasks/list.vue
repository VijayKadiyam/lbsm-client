      ...
      >
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-if="props.column.field == 'added_by'">
            {{
              props.row.added_by
                ? props.row.added_by.first_name +
                  " - " +
                  props.row.added_by.roles[0].name
                : ""
            }}
          </span>
        </template>
      ...
      <b-card>
        ...
        <b-col md="12" class="no-break">
          <table class="my-table">
            ...
            <tr>
              <td>{{ total_completed_task ? total_completed_task : 0 }}</td>
              <td>
                {{
                  total_pending_program_tasks ? total_pending_program_tasks : 0
                }}
              </td>
              <td>{{ average_score ? average_score.toFixed(1) : 0 }}</td>
            </tr>
          </table>
        </b-col>
        <br />
        <b-col md="12" class="no-break">
          <h4>Pending CPP Tasks</h4>
          <table class="my-table">
            <tr>
              <th>Sr No</th>
              <th colspan="4">Program Tasks</th>
            </tr>
            <tr
              v-for="(pendingProgramTask, at) in user_reports[0]
                .pending_user_program_task"
              :key="`pendingProgramTask${at}`"
            >
              <td>{{ at + 1 }}</td>
              <td colspan="4">
                {{
  ...
  methods: {
    ...
    async generatePDF(user_id) {
      ...
      if (this.user_reports != 0) {
        ...
        // Score Details
        columns = [
          { title: "Task Completed", dataKey: "total_completed_task" },
          { title: "Pending Task", dataKey: "total_pending_program_tasks" },
          { title: "Average Score", dataKey: "average_score" },
        ];
        rows = [];
        count = 1;
        // for (const userProgramDetail of this.user_reports[0].user_programs) {
        rows.push({
          total_completed_task: this.total_completed_task
            ? this.total_completed_task
            : "0",
          total_pending_program_tasks: this.total_pending_program_tasks
            ? this.total_pending_program_tasks
            : "0",
          average_score: this.average_score ? this.average_score.toFixed(1) : 0,
          //   85 : "0",
        });
        // count++;
        // }
        doc.text("Score", 10, doc.lastAutoTable.finalY + 10);
        doc.autoTable(columns, rows, {
          startY: doc.lastAutoTable.finalY + 15,
          margin: [35, 10, 15, 10],
          styles: { overflow: "linebreak" },
        ...
        rows = [];
        count = 1;
        for (const pendingUserProgramTask of this.user_reports[0]
          .pending_user_program_task) {
          rows.push({
            sr_no: count,
            task_name:
              pendingUserProgramTask.serial_no +
              " - " +
              pendingUserProgramTask.task,
          });
          count++;
        }
        var strArr = doc.splitTextToSize(
          "A longer title that might be split",
          10
        );
        doc.text(strArr, 10, doc.lastAutoTable.finalY + 20);
        // doc.text("Pending CPP Tasks", 10, doc.lastAutoTable.finalY + 10, );
        doc.autoTable(columns, rows, {
          startY: doc.lastAutoTable.finalY + 15,
          margin: [35, 10, 15, 10],
          styles: { overflow: "linebreak" },
        ...
        doc.text("KARCO Tasks", 10, doc.lastAutoTable.finalY + 10);
        doc.autoTable(columns, rows, {
          startY: doc.lastAutoTable.finalY + 15,
          margin: [35, 10, 15, 10],
          styles: { overflow: "linebreak" },
          ...
          didDrawPage: function (data) {
            ...
            // doc.text("KARCOS Tasks", 10, doc.lastAutoTable.finalY + 30);
            // doc.autoTable(columns, rows, {
            //   startY: doc.lastAutoTable.finalY + 15,
            //   margin: [35, 10, 15, 10],
            //   styles: { overflow: "linebreak" },
